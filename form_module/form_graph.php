                            ...
                            <div class="table-responsive">
                                <table class="table  table-bordered text-nowrap mb-0" id="example2">
                                    ...
                                    if ($station != "" && $datefrom != "" && $dateto != "") {
                                        ...
                                        $result = "SELECT distinct `form_name`,`form_type`,`form_create_id`,station FROM `form_user_data` WHERE DATE_FORMAT(`created_at`,'%Y-%m-%d') >= '$date_from' and DATE_FORMAT(`created_at`,'%Y-%m-%d') <= '$date_to' and station = '$station' " . $q_str . "order by form_type asc";
                                        $qur = mysqli_query($db,$result);
                                    } else if ($station != "" && $user != "" && $datefrom == "" && $dateto == "") {
                                        $qur = mysqli_query($db, "SELECT distinct `form_name`,`form_type`,`form_create_id`,station FROM `form_user_data` WHERE  station = '$station' ");
                                    } else if ($station != "" && $user == "" && $datefrom != "" && $dateto != "") {
                                        $date_from = convertMDYToYMD($datefrom);
                                        $date_to = convertMDYToYMD($dateto);
                                        $qur = mysqli_query($db, "SELECT distinct `form_name`,`form_type`,`form_create_id`,station FROM `form_user_data` WHERE DATE_FORMAT(`created_at`,'%Y-%m-%d') >= '$date_from' and DATE_FORMAT(`created_at`,'%Y-%m-%d') <= '$date_to' and station = '$station' " . $q_str . "order by form_type asc");
                                    } else if ($station != "" && $user == "" && $datefrom == "" && $dateto == "") {
                                        $qur = mysqli_query($db, "SELECT distinct `form_name`,`form_type`,`form_create_id`,station FROM `form_user_data` WHERE station = '$station'");
                                    } else if ($station == "" && $user != "" && $datefrom != "" && $dateto != "") {
                                        $date_from = convertMDYToYMD($datefrom);
                                        ...
                                        $date_from = convertMDYToYMD($datefrom);
                                        $date_to = convertMDYToYMD($dateto);
                                        $qur = mysqli_query($db, "SELECT distinct `form_name`,`form_type`,`form_create_id`,station FROM `form_user_data` WHERE DATE_FORMAT(`created_at`,'%Y-%m-%d') >= '$date_from' and DATE_FORMAT(`created_at`,'%Y-%m-%d') <= '$date_to' ");
                                    }
                                    while ($rowc = mysqli_fetch_array($qur)) {
                                        $form_id = $rowc["form_create_id"];
                                        $query1  = mysqli_query($db, "SELECT part_number,form_comp_status,form_status, created_at , form_user_data_id FROM `form_user_data` where form_create_id = ' $form_id' LIMIT 1");
                                        $rowc1 = mysqli_fetch_array($query1);
                                        $pno = $rowc1['part_number'];
                                        $form_user_data_id = $rowc1["form_user_data_id"];
                                        ...
                                        <tr>
                                            ...
                                            <td>
                                                <a href="form_item_data_view.php?id=<?php echo $rowc['form_create_id']; ?>&station=<?php echo $rowc['station'];; ?>&date_from=<?php echo $datefrom; ?>&date_to=<?php echo $dateto; ?>"
