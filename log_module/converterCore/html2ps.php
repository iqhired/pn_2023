...
/*
$toKill = split("\n", file_get_contents("toKillBefore.log"));
foreach ($toKill as $kllFile) {
	@unlink('./temp/'.trim($kllFile));
}
//*/

//$fp = fopen("../aaaa.log", 'wb');
//fwrite($fp, $pictureFileName."\n".$htmlFileName);
//fclose($fp);

@unlink('./temp/'.trim($_GET['toKill1']));
@unlink('./temp/'.trim($_GET['toKill2']));
